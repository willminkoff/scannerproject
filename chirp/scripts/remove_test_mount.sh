#!/bin/bash
# chirp/scripts/remove_test_mount.sh — remove /CHIRP_TEST.mp3 from icecast.xml.
#
# Reverse of add_test_mount.sh. Idempotent: a no-op if the mount isn't there.
...
# Remove (optional surrounding comment) + the <mount> block whose <mount-name>
# matches. Use a non-greedy regex; XML elements are well-formed here.
mount_pat = re.compile(
    r"(\n[ \t]*<!--[^>]*chirp Phase 3 test mount[\s\S]*?-->)?"
    r"\n[ \t]*<mount>\s*"
...
if n == 0:
    # Fallback: simpler regex if comment shape doesn't match.
    simple = re.compile(
