...
@author: madengr
"""
# -- chirp port note (2026-06-03) --
# Vendored from github.com/madengr/ham2mon @ db9834c (apps/scanner.py).
# GR 3.7 -> GR 3.10 port: 2to3 (print-as-function only; no firdes/window usage here).
# Imports rewritten for package layout (chirp.dsp.ham2mon.*).
# DEFERRED: estimate and parser are NOT vendored in this commit (per scaffold brief).
# Imports are stubbed so the module imports cleanly; any function in scanner.py that
# touches `estimate` or `prsr` will raise AttributeError until Phase 1 vendors them.
...
from chirp.dsp.ham2mon import receiver as recvr  # ported (GR 3.10)

# Deferred until Phase 1: ham2mon's apps/estimate.py and apps/parser.py.
# Stubbed as None so the module imports; code paths that use them will fail loudly.
estimate = None
prsr = None
