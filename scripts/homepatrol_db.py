...
def _extract_area_mapping(
    ...
    """Parse HPDB AreaState / AreaCounty records.

    Returns:
        (entity_kind, entity_id, state_id, county_id)

    Notes:
      - Some HPDB dumps encode AreaCounty lines as two CountyId=... kv pairs.
        In that case, we treat the first as the "entity" and the last as the
        mapped county_id. (In many files they are identical.)
      - If StateId is present on AreaCounty lines, we capture it.
      - If an AreaCounty line includes another entity id (e.g. TrunkId=...),
        that wins as entity_kind/entity_id and CountyId becomes the county_id.
    """
    ...
    if record_type == "AreaCounty":
        county_vals: list[int] = []
        for key, value in pairs:
            if key == "StateId":
                state_id = _to_int(value)
                continue
        ...
        if county_vals:
            county_id = county_vals[-1]

        # Some HPDB files encode AreaCounty as CountyId=<entity>	CountyId=<county>.
        # If we didn't find a different entity id, treat it as a county self-map.
        if entity_kind is None and county_vals:
            entity_kind = "CountyId"
            entity_id = county_vals[0]
