...
2. `LIBACARS_BACKEND_BIN` or adjacent `scripts/libacars_backend` production binary
3. `LIBACARS_HELPER_BACKEND` dynamic Python backend (`module` or `module:attr`)
...
class _BinaryBackend:
    ...
    def _invoke(self, mode: str, payload: dict[str, Any]) -> dict[str, Any] | None:
    ...
    def decode_message(self, payload: dict[str, Any]) -> dict[str, Any] | None:
        return self._invoke("message", payload)

    def decode_vdl2_frame(self, payload: dict[str, Any]) -> dict[str, Any] | None:
        return self._invoke("vdl2", payload)
...
def _binary_backend_from_env() -> Any:
    backend_cmd = str(os.getenv("LIBACARS_BACKEND_BIN", "")).strip()
    if backend_cmd:
        return _BinaryBackend(shlex.split(backend_cmd))

    default_path = pathlib.Path(__file__).resolve().with_name("libacars_backend")
    if default_path.is_file() and os.access(default_path, os.X_OK):
        return _BinaryBackend([str(default_path)])
    return None
...
def _build_backend() -> Any:
    ...
    binary_backend = _binary_backend_from_env()
    if binary_backend is not None:
        return binary_backend
