...
def _generate_multi_rx_start_script(process_specs: list[dict[str, object]]) -> str:
    ...
    if len(specs) == 1:
        ...
        # Phase C reliability: route through op25-supervisor.py instead of
        # exec'ing multi_rx.py directly.  Supervisor adds USB-serial
        # pre-flight + startup deadline + runtime silent-log watchdog so
        # the "active but flowgraph never produced output" wedge gets
        # killed fast and systemd restarts cleanly.
        lines = [
            "#!/bin/bash",
            "# Auto-generated by ensure-op25-runtime.py — supervised multi_rx.py mode",
            f'export PYTHONPATH="{apps_dir}:{tdma_dir}:${{PYTHONPATH:-}}"',
            f'export OP25_CONFIG="{config_path}"',
            f'export OP25_MULTI_RX_PATH="{OP25_MULTI_RX_PATH}"',
            f'cd "{apps_dir}"',
            'exec python3 /home/ubuntu/scannerproject/scripts/op25-supervisor.py 2>&1',
        ]
        return "\n".join(lines) + "\n"
