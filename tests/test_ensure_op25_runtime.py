...
class EnsureOp25RuntimeTests(unittest.TestCase):
    ...
    def test_bootstrap_prefers_tuner_1_across_two_physical_rspduos(self):
        with tempfile.TemporaryDirectory() as tmp:
            tmp_path = Path(tmp)
            profiles_dir = tmp_path / "profiles"
            active_profile = profiles_dir / "hp3_favorites_digital"
            ...
            self.assertEqual("rtl=2", inst0["devices"][1]["args"])
            self.assertEqual(
                "soapy=,driver=sdrplay,serial=1809063632,mode=ST,tuner=1",
                inst1["devices"][0]["args"],
            )
