...
def build_aironly_config(airband_path: str, mixer_name: str, publish: bool = True) -> str:
    with open(airband_path, "r", encoding="utf-8", errors="ignore") as f:
        airband_text = f.read()
    airband_disabled = bool(RE_UI_DISABLED.search(airband_text))
    ...
    top_lines = []
    seen = set()
    for line in extract_top_level_settings(airband_text):
        if line not in seen:
            seen.add(line)
            top_lines.append(line)

    device_payloads = []
    if not airband_disabled:
        payload = extract_devices_payload(airband_text)
        if payload:
            payload = enforce_device_index(payload, SCANNER1_INDEX)
            payload = enforce_device_serial(payload, SCANNER1_RTL_DEVICE)
            payload = replace_outputs_with_mixer(payload, mixer_name)
            device_payloads.append(payload.strip().rstrip(","))

    if publish:
        icecast_block = extract_icecast_block(airband_text)
        if not icecast_block:
            icecast_block = (
                "{\n"
                "  type = \"icecast\";\n"
